...
现在项目也包含一个基于 LangChain + LangGraph 的通用电商客服 CLI Agent。
...
如果要运行电商客服 CLI，除 `DASHSCOPE_API_KEY` 外，还需要大模型可用的同一套 DashScope 环境配置。

## 电商客服 CLI

该 Agent：

- 使用 `ChatTongyi(model="qwen3-max")`
- 使用 LangChain tool + LangGraph 编排
- 直接调用现有 `RAGService.search()`
- 不读取 `docs/` 文件内容做硬编码猜测，只把 RAG 作为知识来源
