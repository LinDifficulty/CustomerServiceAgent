...
|---|---|---|---|
| 聊天 / Agent | `tongyi` | `qwen3-max-2026-01-23` | 通过 LangChain `ChatTongyi` 调用，使用默认配置时需要 `DASHSCOPE_API_KEY` |
...
├── uv.lock                           # uv 锁定依赖
├── main.py                           # 兼容入口，转发到 rag_server.cli:main
...
└── rag_server/
    ...
    ├── rag_service.py                # 文档入库、FAISS、BM25、混合检索、精排
    ├── cli.py                        # LangGraph CLI Agent、命令行参数、交互循环
...
flowchart LR
    ...
    User["CLI 用户输入"] --> CLI["rag-cli / cli.py"]
...
| `model` | `"qwen3-max-2026-01-23"` | `str` | 主 Agent 聊天模型名称 |
...
Redis 缓存默认关闭。启用后会缓存 query rewrite、query embedding、检索结果、rerank 结果和短 TTL 的记忆检索；Redis 连接失败或运行期异常都会降级为 cache miss。
各 TTL 字段设为 `0` 时表示不写入对应类别缓存。

| 字段 | 默认值 | 类型 | 作用 |
|---|---|---|---|
| `enabled` | `false` | `bool` | 是否启用 Redis 缓存 |
...
| `DEFAULT_CHAT_MODEL` | `"qwen3-max-2026-01-23"` | 默认聊天模型 |
...
这些命令不会发给 Agent，`handle_memory_command()` 会直接处理：
...
6. [rag_server/memory_service.py](./rag_server/memory_service.py)
7. [rag_server/query_rewrite.py](./rag_server/query_rewrite.py)
8. [rag_server/reflection_service.py](./rag_server/reflection_service.py)
9. [rag_server/skill_service.py](./rag_server/skill_service.py)
10. [rag_server/mcp_service.py](./rag_server/mcp_service.py)
11. [rag_server/trace_service.py](./rag_server/trace_service.py)
12. [rag_server/eval_service.py](./rag_server/eval_service.py)
13. [tests/](./tests)
