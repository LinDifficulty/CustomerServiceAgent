...
- CLI 当前固定使用 `data/` 与 `memory/` 目录，尚未暴露对应参数
...
2. 根据 `chunk_size` 与 `chunk_overlap` 切分
3. 生成 `doc_id` 与 `chunk_id`
4. 如果文档内容与切分参数未变化，则跳过
5. 如果变化，则删除旧 chunk 并重新生成
6. 重建 FAISS 索引与 BM25 语料
7. 持久化索引和元数据
...
- 长期记忆会落盘到 `memory/`
- `data` 和 `memory` 路径当前在 `run_cli_async()` 内写死
- `--cross-encoder` 默认关闭，只有显式开启时才会触发精排模型加载
- Agent、query rewrite、memory extractor 共用 `LLMRetryPolicy`
...
### 6.3 `rag_server/memory_service.py`
...
### 6.4 `rag_server/query_rewrite.py`
...
### 6.5 `rag_server/skill_service.py`
...
### 6.7 `rag_server/trace_service.py`
...
目前 RAG、Agent、Skill、Memory、Eval 都可以接入 Trace。LLM 重试失败、query rewrite 降级、Agent 工具循环保护也会记录为 warning 或 error 级别事件。

### 6.8 `rag_server/llm_retry.py`
...
### 6.9 `rag_server/eval_service.py` 与 `rag_server/eval_runner.py`
...
6. `rag_server/llm_retry.py`
7. `rag_server/skill_service.py`
8. `rag_server/mcp_service.py`
9. `rag_server/trace_service.py`
10. `rag_server/eval_service.py`
11. `tests/`
