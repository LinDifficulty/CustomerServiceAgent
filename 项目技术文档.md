...
| 包管理 | `uv` |
| 向量检索 | `faiss-cpu` |
| Embedding | `model_factory` 创建，默认 `DashScopeEmbeddings` (`text-embedding-v4`) |
| 关键词检索 | `rank-bm25` (`BM25Plus`) + `jieba` |
| 精排 | `model_factory` 懒加载，默认 `sentence-transformers` `CrossEncoder` (`BAAI/bge-reranker-v2-m3`) |
| LLM | `model_factory` 创建，默认 `ChatTongyi` / `qwen3-max-2026-01-23` |
...
- 默认模型能力依赖 DashScope / Tongyi；聊天、embedding、rerank 和 query rewrite 均可通过 provider + model 替换
...
- `AgentSettings`：聊天模型 provider / model / kwargs、`user_id`、工具循环保护、Reflection 开关
- `RetrievalSettings`：query rewrite、BM25、CrossEncoder 开关，以及 embedding / reranker provider、model 和加载参数
- `LLMSettings`：rewrite / memory provider、模型、构造参数、重试次数、单次超时、退避时间
- `MemorySettings`：长期记忆开关与每层召回数量
- `SkillsSettings`：Skills 开关与额外 Skills 目录
- `MCPSettings`：MCP 开关
- `TraceSettings`：JSONL trace 与 CLI 实时事件开关
- `CLISettings`：CLI 启动配置摘要输出开关
...
配置加载会拒绝未知 section 和未知 key，避免拼写错误静默失效；同时兼容一批旧字段别名，例如 `retrieval.query_rewrite_mode`、`retrieval.bm25_enabled`、`agent.reflection`、`trace.live_events`、`trace.live_logs`、`cli.live_events` 和 `cli.show_startup_config`。

#### 运行时映射

`AppConfig.to_runtime_kwargs()` 会把配置映射成 `run_cli()` 需要的参数。`llm.rewrite_provider` / `llm.rewrite_model` 和 `llm.memory_provider` / `llm.memory_model` 为空时会回落到 `agent` 的聊天模型配置，保证只配置一个主模型也能完整启动。
